...
//		}else{
            ...
            cache('access_token',$this->access_token,1600);
//		}
	}
	...
	//获取永久二维码
	function get_qr_limit($scene_id){
	    //scene_id最大是100000
		$a = $scene_id;
		$url="https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=".$this->access_token;
		$data='{ 
		...
		$surl="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=".$ticket;
		$ress=$this->http_request($surl);
		$file_name = '/uploads/qr/qr/'.$scene_id.'.jpg';
		file_put_contents('uploads/qr/qr/'.$scene_id.'.jpg',$ress);
		return $file_name;
	}
    //微信支付加密字符串
...
//		file_put_contents('e4re.txt',$arr);
        return $arr;
    }
    //获取预支付交易会话标识，有效期两个小时
    function get_prepay_idd($openid,$total_fee,$out_trade_no,$notify_url,$pay_id,$good_name){
        $nonce_str = $this->createNonceStr();
        $sign = $this->signjiami1($openid,$nonce_str,$total_fee,$out_trade_no,$notify_url,$pay_id,$good_name);
        ...
        //file_put_contents('./test_dir/a.txt',$result);
        $postObj = simplexml_load_string($result, 'SimpleXMLElement', LIBXML_NOCDATA);
        $prepay_id = trim($postObj->prepay_id);
        return $prepay_id;
    }
    public function signjiami1($openid,$nonce_str,$total_fee,$out_trade_no,$notify_url,$pay_id,$good_name){
	...
	function save_head_pic($user_id){
		$userinfo=db('user')-> field('headimgurl,nickname') -> where(['user_id' => $user_id])->select();
		$url=$userinfo[0]['headimgurl'];
		$res=$this->http_request($url);
		if(!is_dir('uploads/qr/head_img/')){
			mkdir('uploads/qr/head_img');
		}
		file_put_contents('uploads/qr/head_img/'.$user_id.'.jpg',$res);
